...
cd src/tree-sitter-rgbds && npx tree-sitter generate  # Regenerate parser after grammar.js changes
cd src/tree-sitter-rgbds && npx tree-sitter test      # Grammar corpus tests

cd src/server && npm run build   # Build server (required before integration tests)
cd src/server && npm test        # Unit + integration tests (vitest)
cd src/server && npx vitest run tests/unit.test.ts     # Single test file

cd src/vscode && npm run build   # Build extension
```
...
src/vscode                    src/server               src/tree-sitter-rgbds
```
...
Version locations: `src/*/package.json`, `src/tree-sitter-rgbds/tree-sitter.json`, `.claude-plugin/plugin.json` (bump with higher of server/extension).
